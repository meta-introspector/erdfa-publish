...
use erdfa_publish::stego::{PngLsb, WavPhase, ZeroWidthText, RsHexComment};
use std::time::Instant;

fn all_plugins() -> Vec<Box<dyn StegoPlugin>> {
    vec![Box::new(PngLsb), Box::new(WavPhase), Box::new(ZeroWidthText), Box::new(RsHexComment)]
}
...
fn main() {
    ...
    println!("{}", "-".repeat(55));
    let names: Vec<&str> = vec!["png", "wav", "text", "rs"];
